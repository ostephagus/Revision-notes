...
\begin{theorem}[Steinitz Exchange Lemma]
    Let $S, T$ be finite subsets of a vector space $V$. Let $S$ be linearly independent and let $T$ span $V$. Then:
    \begin{enumerate}
        \item $|S| \leq |T|$;
...
\end{theorem}
\begin{proof}[based on previous theorems]
    Combine \thmref{thmIsomorphism} and \thmref{lemDimQuotient} for the result.
\end{proof}
\begin{proof}[working from bases]
    Consider a basis $B$ of $V$ extended from a basis of $\ker(\alpha) \subseteq V$. Apply $\alpha$ to this basis and take the non-zero vectors to form $C$. Prove that this is a basis of the image.
\end{proof}
