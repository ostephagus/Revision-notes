...
\begin{itemize}
    ...
    \item The total energy operator is the Hamiltonian operator $\hat{H} = (\hat{T} + \hat{U})$.
\end{itemize}
\begin{proposition}
...
Consider 1D and a separable solution $\psi(x, t) = T(t) \chi(x)$. Then we find the time-independent Schr\"odinger Equation (TISE):
\begin{equation}
    -\frac{\hbar^2}{2m} \frac{\partial^{2}\chi}{\partial x^{2}} + U(x) \chi(x) = E\chi(x)
    \label{eqnTISE}
\end{equation}
where $E$ is the energy eigenvalue. The corresponding time solution is $T(t) = e^{-iEt/\hbar}$. The TISE can also be written $\hat{H}\chi = E\chi$.

A solution of this form is a \underline{stationary state}. Here $\rho(x, t) = \abs{\chi(x)}$ is unchanging in time.

Because $\hat{H}$ is a self-adjoint operator, it has a complete basis of eigenfunctions and so every solution of the TDSE can be written as a linear combination of stationary states.
\end{document}